...
/**
 * Produce a colorful rainbow
 * @param str The input text
 * @param clrArray A array of valid colors, if null default to rainbow
 * @returns {string}
 */
exports.rainbow = function (str, clrArray) {
    if (!clrArray) {
        clrArray = ['red', 'olive', 'yellow', 'green',
            'blue', 'navy', 'violet'];
    }
    var l = clrArray.length;
    var i = 0;

    return str
        .split('')
        .map(function(c) {
            return c !== ' ' ? exports[clrArray[i++ % l]](c) : c;
        })
        .join('');
};
